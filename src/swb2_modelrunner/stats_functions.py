...
# should be possible
    ...
    match summary_type:
        ...
        case 'seasonal_mean':
            ...
            result_dataset = xarray_dataset.resample(time="QS-DEC").reduce(np.mean, dim="time")
        ...
        case 'annual_sum':
            ...
            result_dataset = xarray_dataset.resample(time="YE").reduce(np.nansum, dim="time")
        ...
        case 'annual_mean':
            ...
            result_dataset = xarray_dataset.resample(time="YE").reduce(np.nanmean, dim="time")
        ...
        case 'mean_annual_sum':
            ...
            result_dataset = xarray_dataset.resample(time="YE").reduce(np.nansum, dim='time').reduce(np.nanmean, dim="time")

        case 'mean_annual_mean':
            # return a single grid representing the mean of each annual mean variable amount over all years
            result_dataset = xarray_dataset.resample(time="YE").reduce(np.nanmean, dim='time').reduce(np.nanmean, dim="time")
